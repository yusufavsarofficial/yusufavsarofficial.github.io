 ...
 *  0. TEMA YÜKLEYİCİ
 * ------------------------------------------------------------------------
 * Sayfa içeriği yüklenmeden önce çalışarak tema titremesini (flicker) önler.
 */
(() => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleProjectFiltering = () => {
        const categoryContainer = document.querySelector('.filter-buttons');
        ...
        const applyFilter = (filterValue, filterType) => {
            ...
            const activeButton = document.querySelector(`.btn-filter[data-filter="${filterValue}"]`);
            ...
            grid.querySelectorAll('.project-card').forEach(card => {
                ...
                if (filterValue === 'all') {
                    ...
                    matches = cardTags && cardTags.split(',').includes(filterValue);
                }
        ...
        const setupFilterButtons = (container, filterType) => {
        ...
        setupFilterButtons(categoryContainer, 'category');
        if (tagContainer) {
            setupFilterButtons(tagContainer, 'tag');
        }

        // URL'den gelen etiket parametresini işle
        const urlParams = new URLSearchParams(window.location.search);
        const tagFromUrl = urlParams.get('tag');
        if (tagFromUrl) {
            // Sayfa yüklendikten sonra filtreyi uygula
            setTimeout(() => applyFilter(tagFromUrl, 'tag'), 100);
        }
    };
    ...
    const handleDynamicBlogListing = async () => {
        ...
        try {
            ...
            // Dinamik kategori filtre butonları oluştur
            if (categoryContainer) {
                const categories = [...new Set(posts.map(post => post.category))]; // Benzersiz kategorileri al
                categories.forEach(category => {
                    if (!category) return; // Boş kategorileri atla
            ...
            // Dinamik etiket filtre butonları oluştur
            if (tagContainer) {
                ...
                const popularTags = Object.keys(tagCounts).sort((a, b) => tagCounts[b] - tagCounts[a]).slice(0, 5);
    ...
    const handleDynamicBlogPost = async () => {
        ...
        try {
            ...
            if (post.tags && post.tags.length > 0 && tagsWrapper) {
                tagsWrapper.innerHTML = '<h4>Etiketler:</h4>';
                post.tags.forEach(tag => {
                    const tagElement = document.createElement('a');
                    tagElement.className = 'post-tag';
                    // Etikete tıklandığında blog sayfasına yönlendir
    ...
    const initApp = async () => {
        ...
        setActiveNavLink();
        handleThemeSwitcher();
        await handleDynamicBlogListing(); // Butonların oluşması için bekle
        handleDynamicBlogPost(); // Tekil blog yazısını dinamik olarak yükle
        handleWavyHeadline(); // Dalga animasyonunu başlat

        handleScrollAnimations();
        handleContactForm();
        handleProjectFiltering(); // Dinamik butonlar oluştuktan sonra filtrelemeyi bağla
    ...
    const handleDynamicBlogListing = async () => {
        ...
        try {
            ...
            // Dinamik filtre butonları oluştur
            if (filterContainer) {
                const categories = [...new Set(posts.map(post => post.category))]; // Benzersiz kategorileri al
                categories.forEach(category => {
                    if (!category) return; // Boş kategorileri atla
    ...
    /**
     * ------------------------------------------------------------------------
     *  XX. DİNAMİK PROJE LİSTELEME
     * ------------------------------------------------------------------------
     * projelerim.html sayfasındaki proje kartlarını content.json dosyasından
     * asenkron olarak yükler ve ekrana basar.
     */
    const handleDynamicProjectsListing = async () => {
        // Sadece projelerim.html sayfasında çalışsın diye kontrol ediyoruz.
        // Bu kontrolü, sayfada sadece projelerim.html'de olan bir elementin varlığıyla yapabiliriz.
        // Örneğin, projelerim.html'deki h2 başlığının içeriğiyle.
        const pageTitle = document.querySelector('.section-title');
        if (!pageTitle || !pageTitle.textContent.includes('Projelerim')) return;
        ...
        try {
            ...
            if (!response.ok) throw new Error('İçerik dosyası yüklenemedi.');
            
            const content = await response.json();
            const projects = content.projects || [];
            
            projectsGrid.innerHTML = ''; // "Yükleniyor..." mesajını temizle

            if (projects.length === 0) {
                projectsGrid.innerHTML = '<p style="text-align: center; color: var(--color-text-secondary); grid-column: 1 / -1;">Henüz gösterilecek bir proje bulunmuyor.</p>';
                return;
            }
            ...
            projects.forEach(project => {
                ...
                projectCard.innerHTML = `
                    ...
                    <a href="${project.link}" class="action-btn">${project.linkText} &rarr;</a>
                `;
                projectsGrid.appendChild(projectCard);
            });
        } catch (error) {
            projectsGrid.innerHTML = `<p style="text-align: center; color: #ff4d4d; grid-column: 1 / -1;">Hata: Projeler yüklenirken bir sorun oluştu. ${error.message}</p>`;
        }
    };
    ...
    const handleDynamicProjectsListing = async () => {
        ...
        // Sadece projelerim.html sayfasında çalışsın diye kontrol ediyoruz.
        const pageTitle = document.querySelector('.section-title');
        if (!projectsGrid || !pageTitle || !pageTitle.textContent.includes('Projelerim')) return;
    ...
    const handleWavyHeadline = () => {
        ...
        // Kelimeleri alıp harflere bölerek yeniden yapılandır
        const words = wavyHeadline.querySelectorAll('span[aria-hidden="true"]');
        wavyHeadline.innerHTML = ''; // İçeriği temizle
        let totalDelay = 0;
        ...
        words.forEach(word => {
            ...
            for (let i = 0; i < text.length; i++) {
                ...
                span.style.animationDelay = `${totalDelay * 0.05}s`;
                wavyHeadline.appendChild(span);
                totalDelay++;
            }
            // Kelimeler arasına boşluk ekle
            if (!word.isEqualNode(words[words.length - 1])) {
                const space = document.createElement('span');
                space.innerHTML = '&nbsp;';
                wavyHeadline.appendChild(space);
            }
        });
    };
    /**
    ...
    const initApp = async () => {
        ...
        handleThemeSwitcher();
        await handleDynamicBlogListing(); // Butonların oluşması için bekle
        await handleDynamicProjectsListing(); // Projeleri dinamik olarak yükle
        await handleDynamicExpertiseAreas(); // Uzmanlık alanlarını yükle
        handleDynamicBlogPost(); // Tekil blog yazısını dinamik olarak yükle
        handleWavyHeadline(); // Dalga animasyonunu başlat
